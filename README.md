...
| Skill | Descrição |
|---|---|
| [`brainstorming`](./brainstorming/) | Fase 0 (opcional): conduz uma sessão de brainstorming de produto/solução a partir de um tema ou ideia vaga e gera uma proposta de produto (pitch inicial) em `docs/brainstorming/<slug>.md`. O pitch alimenta o `/init-dev` ou a constituição do `/sdd`. Consulta `references/tecnicas.md` (SCAMPER, HMW, inversão, analogias) quando o tema pede mais profundidade. |
